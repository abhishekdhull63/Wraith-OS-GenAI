...
export default function SonarTerminal({ defaultTxPayload, onIntelReceived, onLog }: SonarTerminalProps) {
  ...
  const [transmitPayload, setTransmitPayload] = useState(defaultTxPayload || 'NEXUS-ACTUAL: BOURNE PROTOCOL INITIATED.');
  ...
  return (
    <div className="bg-black/40 border border-emerald-500/20 rounded-xl p-4 shadow-[0_0_25px_rgba(16,185,129,0.05)] relative overflow-hidden">
      ...
      <div className="flex items-center gap-2 border-b border-emerald-500/20 pb-2 mb-4">
        <Radio className={`w-4 h-4 ${isTransmitting || isReceiving ? 'text-emerald-400 animate-pulse' : 'text-emerald-600'}`} />
        <h3 className="text-xs font-bold font-mono text-emerald-400 tracking-[0.2em] shadow-emerald-400 drop-shadow-md">SONAR ACOUSTIC MODEM</h3>
      </div>
      ...
      <div className="space-y-3">
        <div>
          <label className="block text-[9px] text-gray-400 font-mono mb-1 uppercase tracking-wider">Payload Injection Block</label>
          <input 
            type="text" 
      ...
      <div className="mt-4 pt-3 border-t border-emerald-500/10 flex items-start gap-2">
        ...
        <p className="text-[8px] text-gray-500 font-mono leading-tight uppercase">
           Acoustic Air-Gap bypass. Transmits data physically over speaker vibrations across ~18-19kHz. Ensure receiver device microphone is active within 5 meters.
        </p>
      </div>
    </div>
  );
