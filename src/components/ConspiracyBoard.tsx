...
interface Node extends Point { id: string; label: string; type: string }
  ...
  return (
     <div className="fixed inset-0 z-[100] bg-[#050505] flex flex-col pt-12 animate-fade-in font-inter">
        <div className="absolute top-6 left-8 flex items-center justify-between right-8 z-10 pointer-events-none">
           <div>
             <h1 className="text-3xl font-bold tracking-[0.2em] text-red-500 uppercase drop-shadow-[0_0_15px_rgba(239,68,68,0.3)]">Link Analysis Board</h1>
             <p className="text-xs text-gray-400 font-mono tracking-widest mt-2 uppercase">Entity graph tracking. Drag blocks to move. SHIFT+Drag to establish hard redline vectors.</p>
           </div>
           <button 
             onClick={onClose} 
        ...
        <div className="flex-1 relative overflow-hidden" 
             onMouseMove={(e) => {
               ...
               if (dragNodeId) {
                 setNodes(ns => ns.map(n => n.id === dragNodeId ? { ...n, x: e.clientX - rect.left, y: e.clientY - rect.top } : n));
               }
             }}
             onMouseUp={() => {
               setDragNodeId(null);
        ...
        >
          ...
          {nodes.map(node => (
             <div 
               ...
               onMouseUp={() => {
                 if (linkingFromId && linkingFromId !== node.id) {
                   ...
                   if (!edges.find(e => (e.from === linkingFromId && e.to === node.id) || (e.from === node.id && e.to === linkingFromId))) {
             ...
             >
                ...
                <div className="font-semibold text-sm line-clamp-2 text-gray-200 leading-tight">
                  {node.label}
                </div>
