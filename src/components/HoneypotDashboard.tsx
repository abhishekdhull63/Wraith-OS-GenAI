// @ts-nocheck — Legacy dashboard, not in active Palantir flow
/**
...
import HoneypotVault from './HoneypotVault';
...
import type { ToolCallResult } from '../hooks/useSecureIntelligence';
...
export default function HoneypotDashboard({
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      const result: ToolCallResult = await intel.analyzeWithTools(
        buildAnalysisPrompt(inputText),
        (chunk) => setAnalysisResult((prev) => prev + chunk),
        { temperature: 0.2, max_tokens: 800 },
      );

      addLog('SUCCESS', `✅ Entity analysis complete [${result.pipelineMs.toFixed(0)}ms]`);

      // If the LLM autonomously called secure_intelligence
      if (result.toolCall && result.savedEntry) {
        addLog(
          'SUCCESS',
      ...
      addLog('ERROR', `❌ Analysis failed: ${intelErr.message}`);
    } finally {
      setIsStreaming(false);
    }
  }, [inputText, intel, addLog]);
