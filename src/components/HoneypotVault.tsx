    ...
    <div className="glass-card flex flex-col h-full overflow-hidden border-t-4 border-t-emerald-500/50">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-100 flex items-center gap-2">
              SecureVault{' '}
              <span className="text-[10px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded uppercase tracking-widest border border-red-500/30">
                Honeypot Active
              </span>
            </h3>
            <p className="text-xs text-gray-500 font-mono">Local Storage Drive (C:)</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden min-h-[400px]">
        ...
        <div className="w-1/3 border-r border-white/5 overflow-y-auto">
          {filtered.map((entry) => (
            ...
            >
              <div className="flex justify-between items-start mb-1">
                <span className="text-xs font-mono font-bold text-gray-300 truncate pr-2">{entry.title}</span>
              </div>
          ...
          {filtered.length === 0 && (
            <div className="p-8 text-center text-sm text-gray-500 font-mono">No files found.</div>
          )}
        </div>
