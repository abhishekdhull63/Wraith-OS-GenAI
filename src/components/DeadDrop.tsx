...
/**
 * DeadDrop — Ephemeral Encrypted P2P Data Tunnel
 *
 * Establishes a serverless, peer-to-peer mesh network using the native
 * WebRTC RTCPeerConnection and RTCDataChannel APIs. Signal exchange is
 * performed manually via copy-paste of SDP offer/answer payloads,
 * eliminating the need for any signaling server infrastructure.
 *
 * Security properties:
 *  - DTLS-SRTP encryption on the data channel (browser-native)
 *  - No relay servers — direct ICE peer connectivity
 *  - Ephemeral session keys — no persistent cryptographic material
 *  - Air-gap compatible when used with QR/optical signaling
 *
 * Exposes a {@link DeadDropHandle} via `forwardRef` for programmatic
 * message dispatch from parent components (e.g., TelemetryLog).
 */
  ...
  const addTerminalLog = useCallback(
    (msg: string) => {
      ...
      if (msg.includes('ESTABLISHED') || msg.includes('FAILED')) {
        onLog?.(msg.includes('FAILED') ? 'ERROR' : 'SUCCESS', `[P2P TUNNEL] ${msg}`);
      } else {
        onLog?.('INFO', `[P2P TUNNEL] ${msg}`);
      }
    },
    [onLog],
  );
    ...
    <div className="glass-card p-6 border-l-4 border-l-orange-500 rounded-xl space-y-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-100 flex items-center gap-2">Encrypted P2P Tunnel</h3>
            <p className="text-xs text-gray-500 font-mono">Serverless Data Transfer</p>
          </div>
        </div>

        {/* Connection Status Pill */}
        <div
          className={`px-3 py-1.5 rounded-full border text-[10px] font-mono font-bold tracking-widest flex items-center gap-2
            ${
        ...
        >
          <div
            className={`w-2 h-2 rounded-full ${connectionStatus === 'connected' ? 'bg-emerald-400 animate-pulse-glow' : connectionStatus === 'connecting' ? 'bg-amber-400 animate-pulse' : 'bg-gray-600'}`}
          ></div>
      ...
      {connectionStatus !== 'connected' ? (
        <>
          ...
          <div className="bg-black/40 rounded-xl p-4 border border-orange-500/20 space-y-4">
            <div className="flex items-start gap-2 text-xs text-orange-400/80 mb-2">
              ...
              <p>
                This protocol bypasses all servers, utilizing a direct WebRTC Data Channel. You must exchange these
                cryptographic Handshake blocks manually with your peer (e.g. via secure USB, QR code, or an alternate
                encrypted channel).
              </p>
            </div>
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                {localOffer && (
                  <div className="space-y-2 animate-slide-up">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <button
                        onClick={() => handleCopy(localOffer, 'offer')}
                    ...
                    <textarea
                      readOnly
                      value={localOffer}
                      className="w-full h-24 bg-black/60 border border-white/10 rounded-lg p-2 text-[10px] font-mono text-gray-400 outline-none resize-none"
                    />
                  </div>
                )}
                ...
                {localOffer && (
                  <div className="space-y-2 animate-slide-up pt-4 border-t border-white/5">
                    <label className="text-[10px] uppercase tracking-widest text-gray-500 font-bold px-1">
                      2. Paste Peer Answer
                    </label>
                    <textarea
                      value={remoteAnswer}
                ...
                {localAnswer && (
                  <div className="space-y-2 animate-slide-up pt-4 border-t border-white/5">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <button
                        onClick={() => handleCopy(localAnswer, 'answer')}
                        className="text-xs text-orange-400 hover:text-orange-300 flex items-center gap-1"
                      >
                    ...
                    <textarea
                      ...
                      className="w-full h-24 bg-black/60 border border-white/10 rounded-lg p-2 text-[10px] font-mono text-gray-400 outline-none resize-none"
                    />
                    <p className="text-[10px] text-gray-500 text-center font-mono pt-2">
                      Provide this block to the Initiator to complete the tunnel.
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="animate-fade-in p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20 text-center">
          <p className="text-sm font-mono text-emerald-400 font-bold tracking-wider">🔗 P2P TUNNEL ACTIVE</p>
      ...
      {/* Terminal Logs */}
      <div
        className="bg-[#0a0a0f] rounded-xl p-3 border border-white/5 h-32 overflow-y-auto font-mono text-[10px] space-y-1"
        ref={logsRef}
      >
              ...
              className={
                log.includes('ERROR') || log.includes('FAILED')
                  ...
                  : log.includes('ESTABLISHED')
                    ...
                    : log.includes('OPEN') || log.includes('TRANSMITTED') || log.includes('INCOMING')
                      ? 'text-cyan-400'
                      : 'text-gray-400'
              }
            >
