  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-black text-red-500 font-mono flex flex-col items-center justify-center p-8">
      <div className="max-w-3xl w-full border-4 border-red-600 bg-red-950/20 shadow-[0_0_50px_rgba(220,38,38,0.3)] p-8 flex flex-col items-center">
         ...
         <h1 className="text-3xl font-black uppercase tracking-[0.2em] mb-2 text-center text-red-500">OPPENHEIMER PROTOCOL ACTIVE</h1>
         <h2 className="text-lg font-bold text-red-400 mb-6 text-center">Master Encryption Key Missing from RAM. Reconstitution Required.</h2>
         ...
         <div className="flex gap-4 mb-4">
            {[0, 1, 2].map(i => {
               ...
               return (
                  <div key={i} className={`w-16 h-16 border-2 flex items-center justify-center transition-all ${isScanned ? 'bg-red-600 border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.8)]' : 'bg-black border-red-900'}`}>
                     {isScanned ? <CheckCircle2 className="w-8 h-8 text-black" /> : <ScanLine className="w-6 h-6 text-red-900" />}
                  </div>
               );
            })}
         </div>
         <p className="text-sm font-bold tracking-widest text-red-400 mb-2">SCANNED HORCRUXES: {scannedShares.size} / 3</p>
