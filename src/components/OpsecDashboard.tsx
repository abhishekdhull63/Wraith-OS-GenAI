...
import { Activity, Battery, BatteryCharging, HardDrive, ShieldCheck, ShieldAlert, Key, Fingerprint } from 'lucide-react';
import { useGhostDrive } from '../hooks/useGhostDrive';
...
export default function OpsecDashboard({ motionActive, faradayActive, vaultEncrypted, onToggleFaraday, onArmOppenheimer }: OpsecDashboardProps) {
  ...
  const { ghostKeyHash, bindGhostDrive, unbindGhostDrive } = useGhostDrive();
  ...
  return (
    <div className="mb-6 p-4 bg-black/40 border border-emerald-500/20 rounded-xl space-y-4 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
      <div className="flex items-center gap-2 mb-3 border-b border-emerald-500/10 pb-2">
        ...
        <h3 className="text-[10px] font-bold text-emerald-400 tracking-widest uppercase shadow-emerald-400/50">OPSEC TELEMETRY</h3>
      </div>
      ...
      <div className="space-y-3 font-mono text-[10px] uppercase tracking-wider">
        ...
        <div className="flex items-center justify-between text-gray-400">
          ...
          <span className={batteryLevel && batteryLevel < 20 && !isCharging ? 'text-red-400 animate-pulse font-bold' : 'text-emerald-400 font-bold'}>
        ...
        <div className="flex items-center justify-between text-gray-400 border-b border-emerald-500/10 pb-3">
          <span className="flex items-center gap-1.5"><HardDrive className="w-3.5 h-3.5" /> VRAM PRESS</span>
        ...
        <div className="space-y-2.5 pt-1">
          <div className="flex items-center justify-between">
            ...
            <span className={`flex items-center gap-1.5 font-bold ${motionActive ? 'text-emerald-400' : 'text-red-400'}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${motionActive ? 'bg-emerald-400 animate-pulse' : 'bg-red-400'}`} />
          ...
          <div className="flex items-center justify-between">
            ...
            <span className={`flex items-center gap-1.5 font-bold ${faradayActive ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className={`flex items-center gap-1.5 font-bold ${vaultEncrypted ? 'text-emerald-400' : 'text-red-400'}`}>
              {vaultEncrypted ? <ShieldCheck className="w-3 h-3" /> : <ShieldAlert className="w-3 h-3 animate-bounce" />}
              {vaultEncrypted ? 'SECURE' : 'EXPOSED'}
            </span>
          </div>

          <div className="flex items-center justify-between pt-2 mt-2 border-t border-emerald-500/10">
            <span className="text-gray-500 flex items-center gap-1.5"><Key className="w-3 h-3" /> GHOST KEY</span>
            {ghostKeyHash ? (
               <button onClick={unbindGhostDrive} className="text-[9px] px-2 py-0.5 bg-red-500/10 text-red-400 border border-red-500/30 rounded hover:bg-red-500/20 transition-colors uppercase">Eject</button>
            ) : (
               <button onClick={async () => {
                 try { await bindGhostDrive(); } catch(e: any) { alert(e.message); }
               }} className="text-[9px] px-2 py-0.5 bg-purple-500/10 text-purple-400 border border-purple-500/30 rounded hover:bg-purple-500/20 transition-colors uppercase shadow-[0_0_10px_rgba(168,85,247,0.2)]">Bind Drive</button>
            )}
          </div>

          <div className="flex items-center justify-between pt-2 mt-2 border-t border-emerald-500/10">
            <span className="text-gray-500 flex items-center gap-1.5"><Fingerprint className="w-3 h-3 text-cyan-500" /> PHANTOM TYPIST</span>
            <button 
               onClick={() => {
                   setIsCalibrating(true);
                   kdRef.current = new KeystrokeDynamics();
                   setCalibPasses(0);
                   setSamples([]);
               }}
               className="text-[9px] px-2 py-0.5 font-bold bg-cyan-950/40 text-cyan-400 border border-cyan-500/30 rounded uppercase shadow-[0_0_10px_rgba(6,182,212,0.1)] hover:bg-cyan-900/40 transition-colors"
            >
               Calibrate Rhythm
            </button>
          </div>
          ...
          <div className="flex items-center justify-between pt-2 mt-2 border-t border-red-900/50">
            <span className="text-gray-500 flex items-center gap-1.5"><ShieldAlert className="w-3 h-3 text-red-500" /> NUCLEAR OPTION</span>
            <button 
      ...
      {isCalibrating && (
         <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center p-6 border border-cyan-500/30 rounded-xl">
             ...
             <h3 className="text-cyan-400 font-mono text-sm uppercase tracking-widest mb-2 font-bold text-center">Biometric Calibration</h3>
             <p className="text-cyan-500/50 text-[10px] font-mono text-center mb-6 max-w-[200px]">Type the master sequence `//nexus` {5 - calibPasses} more times at your natural cadence.</p>
             ...
             <button onClick={() => setIsCalibrating(false)} className="mt-8 text-[9px] text-gray-500 hover:text-red-400 uppercase tracking-widest">Abort</button>
         </div>
      )}
    </div>
  );
