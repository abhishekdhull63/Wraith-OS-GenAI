    ...
    <div className="glass-card flex flex-col h-[600px] border-emerald-500/20 font-mono text-sm relative overflow-hidden">
      ...
      <div className="p-4 border-b border-emerald-500/10 flex justify-between items-center bg-black/40 relative z-10">
        <div className="flex items-center gap-3">
           <MonitorDot className={`w-5 h-5 ${connectionState === 'CONNECTED' ? 'text-emerald-400 animate-pulse' : 'text-gray-500'}`} />
           <div>
             <h3 className="font-bold text-emerald-400 tracking-widest uppercase">Dark Channel</h3>
             <p className="text-[10px] text-emerald-500/50 uppercase tracking-[0.2em] mt-0.5">P2P RAM-Only WebRTC Link</p>
           </div>
        </div>
        
        {connectionState === 'CONNECTED' && (
           <button onClick={handleBurn} className="px-3 py-1.5 flex items-center gap-2 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded border border-red-500/20 transition-all font-bold text-[10px] uppercase tracking-wider">
      ...
      {connectionState !== 'CONNECTED' && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 bg-[#050505] relative z-10">
           ...
           <p className="text-emerald-400 mb-8 max-w-sm text-center text-xs leading-relaxed">Establish an encrypted peer-to-peer data channel. Communication exists solely in local RAM and is never committed to device storage.</p>
           ...
           {localSignal && (
             <div className="w-full max-w-lg mb-6 group relative">
               <p className="text-[10px] text-emerald-500/50 mb-1 uppercase tracking-widest">Your Outbound Handshake (Copy this)</p>
               <textarea 
                 value={localSignal}
                 readOnly
               ...
               />
               <button onClick={() => navigator.clipboard.writeText(localSignal)} className="absolute top-7 right-3 px-3 py-1.5 bg-emerald-500/20 text-emerald-300 font-bold text-[10px] rounded opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-widest">Copy Block</button>
             </div>
           )}
           ...
           <div className="w-full max-w-lg">
             ...
             <div className="flex gap-2">
               ...
               />
               <button onClick={handleConnectToInvite} className="px-5 bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 rounded-lg border border-emerald-500/30 transition-colors flex items-center justify-center">
            ...
            {messages.length === 0 ? (
              ...
              messages.map(msg => (
                <div key={msg.id} className={`flex flex-col max-w-[80%] ${msg.sender === 'me' ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
                  ...
                  <div className={`p-4 rounded-xl text-sm leading-relaxed ${msg.sender === 'me' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-300' : 'bg-white/5 border border-white/10 text-gray-300'}`}>
                    {msg.text}
                  </div>
                </div>
              ))
