...
// import WraithTerminal from './WraithTerminal';
...
export default function DeepCoverDashboard({
  onLock,
  isBooted: _isBooted = true,
  isUnlocked: _isUnlocked = true,
}: DeepCoverDashboardProps = {}) {
  const intel = useSecureIntelligence();
  const { isOnline } = useNetworkStatus();

  // ── Traffic Anomaly Engine State ────────────────────────────────────────────
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    const namesList =
      uniqueNames[0] !== 'No Entities Detected'
    ...
    if (isThreat) {
      ...
      try {
        ...
        addLog(
          'SUCCESS',
          `🔐 AUTO-PRESERVED: Payload secured to Vault. Hash: ${entry.digital_fingerprint.substring(0, 16)}...`,
        );
  ...
  useEffect(() => {
    const handleWraithEvent = (e: any) => {
      ...
      /* Sonar/Argus removed — commands are no-ops */
    };
      ...
      <div className="flex min-h-screen">
        ...
        >
          ...
          <div className="flex items-center gap-3 p-5 border-b border-white/5">
            ...
            {sidebarOpen && (
              <div className="animate-fade-in">
                ...
                <p className="text-xs text-gray-500 font-mono">PS-21 v1.0</p>
              </div>
            )}
          </div>
          ...
          {sidebarOpen && (
            <div className="flex-1 p-5 space-y-6 animate-fade-in overflow-y-auto">
              ...
              {/* Active Mesh Nodes */}
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Active Mesh Nodes</h3>
                <div className="space-y-2">
                  {[
                    { id: 'Node 1', ip: '192.168.1.45', status: 'SECURE' },
                    { id: 'Node 2', ip: '10.0.0.112', status: 'SECURE' },
                    { id: 'Node 3', ip: '172.16.8.203', status: 'SECURE' },
                  ].map((node) => (
                    <div
                      key={node.id}
                      className="flex items-center justify-between p-2.5 rounded-lg bg-black/30 border border-emerald-500/10 font-mono text-[10px]"
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-emerald-400 font-bold tracking-wider">{node.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {/* Zero-Trust Lockdown — Stealthy sidebar placement */}
          {sidebarOpen && (
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <section className="glass-card p-6 space-y-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-100">Traffic Anomaly Engine</h3>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      await saveToLocker(
                        ...
                        'TEXT',
                        'CRITICAL',
                        'Confidential - Budget Cuts (Decoy)',
                        false,
                        true,
                      );
                ...
                "
                >
                  <Ghost className="w-3.5 h-3.5" />[ CREATE HONEYTOKEN TRAP ]
                </button>
              ...
              <section className="glass-card p-6 space-y-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-100">Subnet Packet Sniffer</h3>
                ...
                <div className="p-5 rounded-xl bg-black/30 border border-white/5 space-y-4">
                  ...
                  <div className="flex items-start gap-2 p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
                    ...
                    <p className="text-xs text-gray-500 leading-relaxed">
                      Audio is captured directly into browser memory and processed locally by Whisper WASM. No audio
                      data ever leaves this device.
                    </p>
                  </div>
                </div>
                    ...
                    {transcript && !intel.stt.isTranscribing && (
                      <button
                        onClick={() => {
                          ...
                          addLog('INFO', '📋 Transcript sent to Traffic Anomaly Engine');
                        }}
                        className="
                      ...
                      >
                        ...
                        Send to Traffic Anomaly Engine
                      </button>
                    )}
                  </div>
                )}
            ...
            {/* ══════════════════════════════════════════════════════════════════
              FULL-WIDTH: LOCAL LOG INGESTION (Air-Gapped File Analysis)
              ══════════════════════════════════════════════════════════════════ */}
            <LogAnalyzer onLog={addLog} />
            ...
            <VaultInterrogation
              ...
              onLoadToAnalyzer={(content) => {
                ...
                addLog('INFO', '📋 Vault evidence loaded into Traffic Anomaly Engine');
              }}
              onLog={addLog}
            />
