...
export default function WraithTerminal({ onCommand, onUnlock }: WraithTerminalProps) {
  ...
  const [history, setHistory] = useState<{ text: string; type: 'cmd' | 'system' }[]>([
    { text: 'WRAITH OS PS-21 v1.0 [AUTHORIZED]', type: 'system' },
  ...
  useEffect(() => {
    const handleGlobalKeydown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Backquote' || (e.ctrlKey && e.shiftKey && e.code === 'KeyP')) {
        if (
  ...
  const handleExecute = async (e: React.FormEvent) => {
    ...
    setHistory((h) => [...h, { text: `root@wraith-os:~# ${cmdLine}`, type: 'cmd' }]);
    ...
    if (command === 'chronos' && (args[0] === '--sync' || args[0] === '--calibrate')) {
      setIsCalibrating((prev) => {
        const next = !prev;
        if (next) {
          pushSystemMessage('Initializing Native Geolocation & Magnetometer APIs...');
          pushSystemMessage('Mounting live mesh node telemetry into HUD stream...');
        } else {
          pushSystemMessage('Terminating mesh node telemetry streams...');
        }
        return next;
      });
      return;
    ...
    try {
      if (command === 'help') {
        ...
        await pushSystemMessage('  status                  - Print active subsystem health');
        ...
        await pushSystemMessage('[CHRONOS: SYNCED]');
        await pushSystemMessage('[SENTINEL: ARMED]');
        await pushSystemMessage('[FARADAY CAGE: STANDBY]');
        onCommand('status', []);
  ...
  return (
    ...
    >
            ...
            <div className="mt-6 border border-gray-700 bg-gray-800/40 p-4 rounded-lg flex flex-col gap-2 font-mono text-xs text-slate-300 max-w-[400px]">
              <div className="flex items-center justify-between text-[10px] text-gray-500 border-b border-gray-700 pb-2 mb-1">
                <span className="flex items-center gap-2 font-bold uppercase tracking-widest">
                  <Crosshair className="w-3 h-3" /> CHRONOS TELEMETRY [LIVE]
                </span>
                ...
                <>
                  <div className="flex justify-between items-center py-1">
                    ...
                    <span className="font-bold text-slate-200">
                      {chronosSensors.lat !== null ? chronosSensors.lat.toFixed(6) : 'AQUIRING_SAT...'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="flex items-center gap-2 text-gray-500">
                    ...
                    <span className="font-bold text-slate-200">
                      {chronosSensors.lng !== null ? chronosSensors.lng.toFixed(6) : 'AQUIRING_SAT...'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="flex items-center gap-2 text-gray-500">
                      <Compass className="w-3 h-3" /> MAG_HEADING
                    </span>
                    <span className="font-bold text-slate-200">
                      {chronosSensors.heading !== null
                        ? `${chronosSensors.heading.toFixed(1)}°`
                        : 'CALIBRATING_GYRO...'}
                    </span>
                  </div>
                </>
      ...
      <form
        ...
        className={`flex items-center bg-[#181818] p-3 border-t border-gray-700 gap-2 ${isError ? 'animate-error-shake' : ''}`}
      >
        <span className="text-slate-400 font-bold text-sm whitespace-nowrap">
          <span className="opacity-50">[{timestamp}]</span> root@wraith-os:~#
        </span>
        <div className="flex-1 relative h-5">
          ...
          <div className="flex items-center h-full pointer-events-none">
            <span
              className={`${isError ? 'text-red-500' : 'text-slate-200'} font-mono text-sm whitespace-pre transition-colors`}
            >
              {inputStr}
            </span>
            <span
              className={`${isError ? 'text-red-500' : 'text-cyan-400'} font-mono text-sm transition-colors`}
