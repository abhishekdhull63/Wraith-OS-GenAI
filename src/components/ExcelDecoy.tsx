...
/**
 * ExcelDecoy — Visual Obfuscation & Physical OPSEC Layer
 *
 * Renders a pixel-perfect replica of a corporate spreadsheet application
 * (Microsoft Excel Online) to provide full-screen visual camouflage for
 * the underlying intelligence dashboard. Designed to defeat shoulder-surfing
 * attacks and physical surveillance in hostile environments.
 *
 * Key features:
 *  - Interactive cell grid with editable values and formula bar
 *  - Ribbon toolbar with formatting controls (Bold, Italic, Underline)
 *  - Functional sheet tabs and zoom slider for maximum authenticity
 *  - Real cell selection highlighting with green active-cell border
 *  - All interactions are local — no network activity is generated
 *
 * The decoy is rendered as the primary view when the application state
 * is set to 'decoy', and transitions to the real dashboard upon
 * successful authentication via the hidden unlock mechanism.
 */
...
export default function ExcelDecoy() {
  ...
  const applyPercent = () => {
    setFakeData((prev) => {
      ...
      return {
        ...prev,
        [focusedCell]: (num / 100).toLocaleString('en-US', { style: 'percent', minimumFractionDigits: 1 }),
      };
    });
  };
    ...
    <div className="flex flex-col h-[100vh] w-[100vw] bg-white text-gray-800 font-sans cursor-default select-none overflow-hidden">
      ...
      <div className="flex items-center justify-between bg-[#107c41] text-white px-2 py-1 select-none shadow-md z-10 relative">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm px-3 py-1 bg-white/10 rounded cursor-pointer hover:bg-white/20 transition mix-blend-screen shadow-inner">
            Share
          </span>
        </div>
      </div>
          ...
          <>
            <div className="flex items-center gap-2 pr-4 border-r border-gray-300">
              <div
                ...
                className={`w-6 h-6 border border-gray-300 rounded flex items-center justify-center cursor-pointer shadow-sm transition ${cellStyles[focusedCell]?.fontWeight === 'bold' ? 'bg-gray-300' : 'bg-white hover:bg-gray-200'}`}
              >
                B
              </div>
              <div
                onClick={() => toggleStyle('fontStyle', 'italic', 'normal')}
                className={`w-6 h-6 border border-gray-300 rounded flex items-center justify-center italic cursor-pointer shadow-sm transition ${cellStyles[focusedCell]?.fontStyle === 'italic' ? 'bg-gray-300' : 'bg-white hover:bg-gray-200'}`}
              >
                I
              </div>
              <div
                onClick={() => toggleStyle('textDecoration', 'underline', 'none')}
                className={`w-6 h-6 border border-gray-300 rounded flex items-center justify-center underline cursor-pointer shadow-sm transition ${cellStyles[focusedCell]?.textDecoration === 'underline' ? 'bg-gray-300' : 'bg-white hover:bg-gray-200'}`}
              >
                U
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pr-4 border-r border-gray-300">
              <div
                onClick={() => toggleStyle('textAlign', 'left', 'left')}
                className={`w-6 h-6 border border-gray-300 rounded flex items-center justify-center cursor-pointer shadow-sm transition ${cellStyles[focusedCell]?.textAlign === 'left' ? 'bg-gray-300' : 'bg-white hover:bg-gray-200'}`}
              >
                L
              </div>
              <div
                onClick={() => toggleStyle('textAlign', 'center', 'left')}
                className={`w-6 h-6 border border-gray-300 rounded flex items-center justify-center cursor-pointer shadow-sm transition ${cellStyles[focusedCell]?.textAlign === 'center' ? 'bg-gray-300' : 'bg-white hover:bg-gray-200'}`}
              >
                C
              </div>
              <div
                onClick={() => toggleStyle('textAlign', 'right', 'left')}
                className={`w-6 h-6 border border-gray-300 rounded flex items-center justify-center cursor-pointer shadow-sm transition ${cellStyles[focusedCell]?.textAlign === 'right' ? 'bg-gray-300' : 'bg-white hover:bg-gray-200'}`}
              >
                R
              </div>
            </div>
            <div className="flex items-center gap-2 pr-4 border-r border-gray-300">
              <div
                ...
                className="w-6 h-6 bg-white hover:bg-gray-200 border border-gray-300 rounded flex items-center justify-center cursor-pointer shadow-sm transition text-green-700 font-bold"
              >
                $
              </div>
              <div
                onClick={applyPercent}
                className="w-6 h-6 bg-white hover:bg-gray-200 border border-gray-300 rounded flex items-center justify-center cursor-pointer shadow-sm transition font-bold"
              >
                %
              </div>
            </div>
      ...
      <div className="flex items-center bg-white border-b border-gray-300 text-sm select-none relative z-10 shadow-sm">
        <div className="px-3 border-r border-gray-300 py-1 flex-shrink-0 w-16 text-center shadow-inner font-semibold text-black bg-[#f3f2f1]">
          {focusedCell}
        </div>
        <div className="px-2 border-r border-gray-300 py-1 text-gray-500 hover:bg-gray-100 cursor-pointer flex-shrink-0 transition font-serif italic">
          fx
        </div>
        <input
      ...
      >
        <div className="flex">
          ...
          {cols.map((col) => (
            <div
              key={col}
              className="w-24 flex-shrink-0 bg-[#f3f2f1] border-r border-b border-gray-300 text-center py-1 text-xs text-black shadow-sm font-semibold sticky top-0 z-10 hover:bg-[#e1dfdd] cursor-pointer transition-colors"
            >
              {col}
            </div>
          ))}
      ...
      <div className="flex items-center justify-between bg-[#f3f2f1] border-t border-gray-300 text-xs px-2 py-1 select-none relative z-10 shadow-md">
        <div className="flex items-center">
          ...
          <div className="px-4 py-1.5 bg-white border-b-[3px] border-[#107c41] font-bold text-[#107c41] cursor-pointer shadow-sm z-10 relative">
            Sheet1
          </div>
          ...
          <div className="px-3 py-1.5 text-black hover:bg-gray-200 rounded cursor-pointer ml-2 text-lg leading-none transition">
            +
          </div>
        </div>
        <div className="flex items-center gap-4 text-gray-600 px-4 font-semibold text-black">
          ...
          <span className="flex items-center gap-1 font-mono">
            ...
            <input type="range" className="w-20 accent-[#107c41]" />
