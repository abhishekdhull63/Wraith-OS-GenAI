...
export default function ChronosRadar({
  sensors,
  targetLat,
  targetLng,
  targetHeading,
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[99999] flex flex-col items-center justify-center font-mono">
      ...
      <div className="relative z-10 w-[350px] p-6 border-2 border-emerald-500/30 rounded-full aspect-square flex flex-col items-center justify-center bg-black/50 shadow-[0_0_50px_rgba(16,185,129,0.2)]">
        ...
        <div className="absolute inset-0 rounded-full border border-emerald-500/50 overflow-hidden">
          <div
            className="w-full h-full bg-gradient-to-tr from-emerald-500/0 via-emerald-500/20 to-emerald-500/0 animate-[spin_4s_linear_infinite]"
        ...
        <div className="relative z-20 flex flex-col items-center text-center">
          <Crosshair
            className={`w-12 h-12 mb-4 transition-colors ${isArmed ? 'text-emerald-400 drop-shadow-[0_0_15px_rgba(16,185,129,1)]' : 'text-emerald-900'}`}
          />
          ...
          <div className="w-full space-y-3 text-xs mb-8">
            <div
              className={`flex items-center justify-between px-4 py-2 rounded border ${isLocationLocked ? 'bg-emerald-500/20 border-emerald-400 text-emerald-300' : 'bg-black border-red-900/50 text-red-500'}`}
            >
              <span className="flex items-center gap-2">
                <MapPin className="w-4 h-4" /> LAT/LNG (15m)
              </span>
              <span className="font-bold">{distance !== null ? `${distance.toFixed(1)}m` : 'SCANNING'}</span>
            </div>

            <div
              className={`flex items-center justify-between px-4 py-2 rounded border ${isHeadingLocked ? 'bg-emerald-500/20 border-emerald-400 text-emerald-300' : 'bg-black border-red-900/50 text-red-500'}`}
            >
              <span className="flex items-center gap-2">
                <Compass className="w-4 h-4" /> COMPASS (15°)
              </span>
